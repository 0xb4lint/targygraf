 ...
 *
 * The Laravel-era site stored progress per university subdomain. After the
 * move to apex paths those origins' localStorage would be unreachable, so
 * university and program pages embed a hidden iframe pointing at
 * https://{university}.targygraf.hu/__ls-migrate (a path the redirect layer
 * intentionally leaves alone). That page posts the stored values back here
 * and this script merges them into the apex origin's localStorage, once per
 * university, then reloads so the unmodified targygraf.js picks them up.
 */
(function () {
	'use strict';
